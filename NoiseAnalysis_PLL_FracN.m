function [pll_opt_jit_fracN,pll_opt_ugb1_fracN,pll_opt_ugb2_fracN,phase_margin_1,phase_margin_2] = NoiseAnalysis_PLL_FracN(single_loop,fig_num,fc_vco,fref_fac,fvco_fac,zero_num,pole_num,SD_order,SD_fac,ro_usecase,prescale_div)
...
ref_psd = -160+3; % in dB/Hz

Kref = (10^(ref_psd/10))*((fref/20e6)^2)*(1-ro_usecase);
fc = fc_vco; % Flicker noise corner for VCO noise psd
Kvco = 2*((fvco/2.4e9)^2);
Kvco1 = 2*(1)*((fvco1/2.4e9)^2);
Kvco2 = 2*((fvco2/2.4e9)^2);

% Sigma delta parameters
delta = ((Tvco*prescale_div)/Tref)*2*pi;
z = exp(1j*w_rad*Tref);
L = SD_order;
sigmaDeltaTF = ((abs(1-(z.^(-1)))).^(2*L))./((abs(1-(z.^(-1)))).^2) * (SD_fac);
sigmaDelta_noise_psd = ((delta.^2)./(12.*fref));

delta_casc = (Tvco2/Tvco1)*2*pi;
z_casc = exp(1j*w_rad*Tvco1);
sigmaDeltaTF_casc = ((abs(1-(z_casc.^(-1)))).^(2*L))./((abs(1-(z_casc.^(-1)))).^2);
sigmaDelta_noise_psd_casc = ((delta_casc.^2)./(12.*fvco1));
...
switch single_loop
    case 1
        x=1;
        for fugb = logspace(log10(fref/1000),log10(fref/20),100)
            Wugb = 2*pi*fugb;
            LG_1 = 0;
            LG_2 = 1;
            for L_idx = 1:1:L
            ...
            vco_noise_psdTotal_OL = ((Kvco./(freq.^2)).*(1+(fc./freq)));
            vco_noise_psdTotal = ((Kvco./(freq.^2)).*(1+(fc./freq)).*pll_highPassTF);
            
            ref_noise_psdTotal_OL = (Kref);
            ref_noise_psdTotal = (Kref)*(pll_lowPassTF);
            
            sigmaDelta_noise_psdTotal_OL = (sigmaDelta_noise_psd).*(sigmaDeltaTF);
            sigmaDelta_noise_psdTotal = (sigmaDelta_noise_psd).*(sigmaDeltaTF).*(pll_lowPassTF);

            pll_noise_psdTotal = vco_noise_psdTotal + ref_noise_psdTotal + sigmaDelta_noise_psdTotal;
            ...
            sigmaDelta_powerTotal(x) = trapz(freq(1:length(freq)),sigmaDelta_noise_psdTotal(1:length(freq)));
        ...
        fugb = logspace(log10(fref/1000),log10(fref/20),100);
        ...
        sigmaDelta_cont_atMinJitter = sigmaDelta_powerTotal(pll_MinPowerTotal_idx)/pll_powerTotal(pll_MinPowerTotal_idx);

        pll_opt_jit_fracN = min(pll_jitterTotal);
        pll_opt_ugb1_fracN = fugb(pll_MinJitterTotal_idx)
        pll_opt_ugb2_fracN = "NA";
        figure(fig_num)
        loglog(fugb,1e15*pll_jitterTotal);
        hold on
        xlabel('$f_{u}\,(Hz)$','interpreter','latex')
        ylabel('$J_{rms}\,(fs)$','interpreter','latex')
    case 0
        x=1;
        for fugb2 = logspace(log10(fvco1/1000),log10(fvco1/5),100)
            ...
            for fugb1 = logspace(log10(fref/1000),log10(fref/5),100)
                ...
                vco_noise_psdTotal_loop1 = ((Kvco1./(freq.^2)).*(1+(fc./freq)).*pll_highPassTF_loop1);
                vco_noise_psdTotal_loop2 = ((Kvco2./(freq.^2)).*(1+(fc./freq)).*pll_highPassTF_loop2);
                
                ref_noise_psdTotal_loop1 = (Kref)*(pll_lowPassTF_loop1);
                sigmaDelta_noise_psdTotal = (sigmaDelta_noise_psd_casc).*(sigmaDeltaTF_casc).*(pll_lowPassTF_loop2);
                pll_noise_psdTotal_loop1 = vco_noise_psdTotal_loop1 + ref_noise_psdTotal_loop1;
                pll_noise_psdTotal = pll_noise_psdTotal_loop1.*(pll_lowPassTF_loop2) + vco_noise_psdTotal_loop2 + sigmaDelta_noise_psdTotal;
       ...
       pll_opt_jit_fracN = min(pll_jitterTotal(:));
       [pll_MinJitterTotal_Xidx,pll_MinJitterTotal_Yidx] = find(pll_jitterTotal == pll_opt_jit_fracN);
       ...
       pll_opt_ugb1_fracN = fugb1(pll_MinJitterTotal_Yidx);
       pll_opt_ugb2_fracN = fugb2(pll_MinJitterTotal_Xidx);
